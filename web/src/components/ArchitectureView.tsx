...
export function ArchitectureView({ stages }: { stages: ArchitectureStage[] }) {
  ...
  return <div className="view view--architecture">
    <header className="view-header"><div><span className="section-kicker">Architecture / trust domains</span><h1>A compiler with a verifier outside its trust.</h1><p>Each stage declares what it receives, what it emits, and what happens when its evidence is not enough.</p></div><div className="architecture-legend"><Legend tone="compressor" label="Compressor trust" /><Legend tone="verifier" label="Verifier trust" /><Legend tone="recovery" label="Recovery trust" /><Legend tone="target_evaluation" label="Target evaluation" /></div></header>
