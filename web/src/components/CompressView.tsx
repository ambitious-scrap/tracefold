  ...
  return <div className="view view--compress">
    ...
    <section className="control-plane" aria-labelledby="request-title">
      ...
      <form className="request-form" onSubmit={(event) => { event.preventDefault(); onCompress(); }}>
        ...
        <label className="field field--tokenizer"><span>Measurement source</span><select value={request.tokenizerIdentity} onChange={(event) => onRequestChange({ tokenizerIdentity: event.target.value })}><option>Fixture byte counter · v1</option><option>Configured tokenizer · not connected</option><option>Provider usage · backend only</option></select></label>
        ...
        <label className="field field--editor"><span>Source context <em>· committed fixture bytes</em></span><textarea value={editorText} onChange={(event) => { setEditorText(event.target.value); onRequestChange({ sourceText: event.target.value }); }} spellCheck={false} aria-label="Source context" /></label>
